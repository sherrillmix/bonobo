...
#from ade4 with mclapply replacing lapply
par.mantel.rtest<-function (m1, m2, nrepet = 99,mc.cores=1, ...){
    if (!inherits(m1, "dist")) 
    ...
    mantelnoneuclid <- function(m1, m2, nrepet) {
        ...
        w <- ade4::as.randtest(obs = obs, sim = perm, call = match.call(), 
            ...)
        return(w)
    }
    if (ade4::is.euclid(m1) & ade4::is.euclid(m2)) {
        tab1 <- ade4::pcoscaled(m1)
        obs <- cor(ade4::dist.quant(tab1, 1), m2)
        if (nrepet == 0) 
            return(obs)
        perm <- rep(0, nrepet)
        perm <- unlist(parallel::mclapply(perm, function(x) cor(dist(tab1[sample(n), 
            ]), m2),mc.cores=mc.cores))
        w <- ade4::as.randtest(obs = obs, sim = perm, call = match.call(), 
            ...)
        return(w)
    }
