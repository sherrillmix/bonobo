...
moreThanProp<-apply(otuProp[,samples$name[isEnough[samples$Code]]],1,max,na.rm=TRUE)>.02
cols<-c('white',tail(rev(heat.colors(110)),99)) 
plotProp<-t(otuProp[moreThanProp,samples$name[isEnough[samples$Code]]])
maxProp<-apply(otuProp[moreThanProp,samples$name[isEnough[samples$Code]]],1,function(x)x/max(x))
...
## Calculate p-values 
```{r}
sampleOrder<-withAs(s=samples[isEnough[samples$Code],],s$name[order(!s$bonobo,ifelse(s$area=='KR',FALSE,s$malaria),s$area2,s$malaria)])
tlPos<-samples$name[isEnough[sample$Code]&samples$isTL&samples$malaria]
tlNeg<-samples$name[isEnough[sample$Code]&samples$isTL&!samples$malaria]
#from functions.R
