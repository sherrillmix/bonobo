my.biplot.pcoa<-function (x, Y = NULL, plot.axes = c(1, 2), dir.axis1 = 1, dir.axis2 = 1, rn = NULL,mar= c(4, 4, 1, 0.2), ...) {
  k <- ncol(x$vectors)
  if (k < 2) 
  ...
  if (is.null(Y)) {
    ...
    par(mar = mar)
    myBiplot(x$vectors[, plot.axes], U, xlab = labels[1], ylab = labels[2],...)
  }
