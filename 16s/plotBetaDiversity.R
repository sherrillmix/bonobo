...
pVals$sig<- symnum(pVals$p, corr = FALSE, na = FALSE, cutpoints = c(0, 0.0001, 0.001, 0.01,1), symbols = c("***", "**", "*",''))
...
pdf('out/dists.pdf',width=6,height=8)
  ...
  #boxplot(unlist(distList)~compareFactor,las=1,xlab='UniFrac distance',horizontal=TRUE,range=Inf,notch=TRUE)
  stats<-boxplot(unlist(distList)~compareFactor,las=1,xlab='UniFrac distance',horizontal=TRUE,range=Inf,notch=TRUE,plot=FALSE)
