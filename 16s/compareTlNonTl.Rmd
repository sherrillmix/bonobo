...
tlGroups<-withAs(s=ss[ss$bonobo&isEnough[ss$Code]&ss$isTL,],tapply(s$name,s$area2,c))
nonTlGroups<-withAs(s=ss[ss$bonobo&isEnough[ss$Code]&!ss$isTL,],tapply(s$name,s$area2,c))
...
```{r}
condensePLt<-p.adjust(apply(ninePs,2,fishers,correct=3),'fdr')
summary(condensePLt)
